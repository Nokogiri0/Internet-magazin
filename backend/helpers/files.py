...
def save_file(upload_file: UploadFile, user_id: int, db: Session) -> File | None:
    if not upload_file or not upload_file.filename:
        return
    original_file_name = upload_file.filename
    suffix = Path(original_file_name).suffix
    file_model = CRUDFile(db).create_file(
...
def save_image_in_db_by_url(url: str, user_id: int, db: Session) -> File | None:
    ...
    if r.status_code == 200:
        ...
        buf.seek(0)
        return save_image(upload_file=None, user_id=user_id, db=db, bytes_io_file=buf, resize_image_options=(1000, 1000), detail_error_message="не удалось скачать обложку ролика")
